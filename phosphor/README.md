...
- **Sequential and Diverging tabs:** each keeps its own colors, settings and adjustments. A diverging gradient is two sequential gradients joined at a midpoint color (marked ◆ in the color list, never the first or last color). Each side is corrected separately, so OKLab L runs linearly from the start to the midpoint and back to the start's lightness at the end. If the end color's own lightness differs from the start's, it keeps its hue and chroma and takes the start's lightness, and a note says by how much. **Sort by lightness** orders each side toward the midpoint. The Diverging tab has its own presets: Frostfire and the nine 11-class ColorBrewer diverging schemes.
...
- **Export stop counts are odd** (17 for ArcGIS Pro, 33 for Python and QGIS) so a diverging midpoint always lands exactly on a stop.
- **Hand-adjusted lightness:** drag a dot on the lightness chart up or down (or focus it and use the arrow keys; Shift moves 5) to set that step's OKLab L. Each color keeps the corrected gradient's hue and chroma, and lightness between steps follows a straight line through the dots, so the bar, swatches and every export follow. **Reset lightness** restores the linear curve; changing the colors or interpolation space also clears adjustments, changing the step count keeps their shape, and share links include them. Gradients shown uncorrected can't be adjusted.
- **Output:** the corrected gradient; clickable swatches; an OKLab lightness chart against the linear target, with the uncorrected curve for reference; syntax-highlighted exports as a hex list, a CSS `linear-gradient`, and Python code for a matplotlib colormap (33 samples for a continuous `LinearSegmentedColormap`, plus a `ListedColormap` of the steps).
- **QGIS:** *Download .xml* saves a QGIS style file with one gradient color ramp (33 stops, since QGIS blends linearly in RGB between stops). In QGIS, open **Settings → Style Manager → Import/Export → Import Item(s)** and pick the file. A **Ramp name** field above the exports names the ramp for Python, QGIS and ArcGIS Pro.
- **ArcGIS Pro:** *Download .stylx* saves a style file containing one color ramp: a `CIMMultipartColorRamp` of 17 colors joined by 16 `CIMLinearContinuousColorRamp` segments, which Pro blends in CIELAB. With 17 stops, OKLab lightness stays within about 0.3 (on 0–100) of linear. In Pro, open **Catalog → Styles**, right-click and choose **Add → Add Style**. The ramp then appears in the Symbology pane's color scheme list (turn on **Show names** and **Show all**). The file is built in the browser with [sql.js](https://github.com/sql-js/sql.js), and its layout matches a style saved by ArcGIS Pro 3.1.
