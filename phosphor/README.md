...
- **Output:** the corrected gradient; clickable swatches; an L* chart against the linear target, with the uncorrected curve for reference; syntax-highlighted exports as a hex list, a CSS `linear-gradient`, and Python code for a matplotlib colormap (32 samples for a continuous `LinearSegmentedColormap`, plus a `ListedColormap` of the steps).
